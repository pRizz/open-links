...
Referral links are now a supported maintainer surface. Prefer the repo-native AI CRUD docs first, and use [`skills/referral-management/SKILL.md`](skills/referral-management/SKILL.md) when the work involves reusable referral families, offers, matcher/link shapes, a shared-vs-fork scope decision, or inbox/MCP batch import of referral links from email. Review the official terms/help page before publishing a public referral URL; `bun run referrals:terms:check -- --url <referral-url> [--terms-url <terms-url>]` is the scripted shortcut. If the terms forbid public posting, keep the public page informational and do not expose the personal referral URL. If the public-share policy is missing or ambiguous, confirm with the user before proceeding. The shared catalog in `data/policy/referral-catalog.json` is the higher-level authoring layer, the optional `data/policy/referral-catalog.local.json` file is the fork-owned overlay for local-only additions, and `links[].referral` remains the runtime/render contract with manual overrides. Studio is still a fallback for referral edits only when Advanced JSON is acceptable, and direct JSON edits stay as the lower-level path. The canonical referral contract lives in [docs/data-model.md](docs/data-model.md), and the script-backed referral verification checklist lives in [docs/social-card-verification.md](docs/social-card-verification.md).
...
- Opt-in profile vCard download with explicit contact-field, photo, and URL allowlists.
...
- Preferred for repo-native maintenance: use the repo's AI workflows/skills through [OpenClaw Update/CRUD Contract](https://raw.githubusercontent.com/pRizz/open-links/main/docs/openclaw-update-crud.md), [OpenClaw Bootstrap Contract](https://raw.githubusercontent.com/pRizz/open-links/main/docs/openclaw-bootstrap.md), [AI-Guided Customization Wizard](https://raw.githubusercontent.com/pRizz/open-links/main/docs/ai-guided-customization.md), [Linktree Bootstrap Extractor](https://raw.githubusercontent.com/pRizz/open-links/main/docs/linktree-bootstrap.md), [`skills/rename-social-profile/SKILL.md`](skills/rename-social-profile/SKILL.md) when a verified same account changed its handle or username and must retain analytics history, [`skills/referral-management/SKILL.md`](skills/referral-management/SKILL.md) when referral catalog families/offers/matchers, fork-vs-upstream catalog scope, or inbox/MCP batch import need an interview-driven workflow, [`skills/cache-rich-link-assets/SKILL.md`](skills/cache-rich-link-assets/SKILL.md) when rich-link image assets need to be committed, and [`skills/openlinks-fork-identity-presence/SKILL.md`](skills/openlinks-fork-identity-presence/SKILL.md) when you want other websites, repos, docs, or services to point back to your deployed OpenLinks fork.
- For referral links: prefer the repo-native AI CRUD docs and the referral-management skill first, including inbox/MCP batch import when the user wants referral codes mined from email; review the official terms/help page before publishing a public referral URL, use `bun run referrals:terms:check -- --url <referral-url> [--terms-url <terms-url>]` for the scripted check, do not publish URLs whose terms forbid public posting, and confirm with the user when the public-share policy is missing or ambiguous. Use Studio only when Advanced JSON fits the change, treat `data/policy/referral-catalog.local.json` as fork-owned overlay data, and use `docs/data-model.md` as the canonical runtime field reference.
...
- Preferred for browser-based CRUD: use [OpenLinks Studio](https://raw.githubusercontent.com/pRizz/open-links/main/docs/studio-self-serve.md) when the self-serve onboarding/editor already covers your workflow. vCard download and photo export have guided controls; referral editing currently relies on Advanced JSON.
...
- [`skills/rename-social-profile/SKILL.md`](skills/rename-social-profile/SKILL.md): verify and apply same-account social handle changes while preserving the stable link ID, capturing a fresh cache identity, and appending to the existing follower-history CSV without rewriting historical handles or URLs.
...
- [`skills/referral-management/SKILL.md`](skills/referral-management/SKILL.md): interview-driven referral catalog CRUD, official-terms public-sharing guardrails, inbox/MCP referral import planning, link-level `catalogRef` adoption, fork-local overlay decisions, and upstream-PR hygiene that keeps `data/policy/referral-catalog.local.json` out of shared diffs.
...
- `bun run social:profile:rename -- --link-id <id> --new-url <url>` - print a read-only same-account social-handle rename plan; add `--apply --confirm-same-account` only after identity confirmation. The command preserves the link ID and never edits follower-history files.

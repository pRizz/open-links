...
## task-upstream-spotify-rich-card | 2026-04-05 06:08 CDT | Add upstream Spotify rich card support and upstream auto-enrichment rule

- [x] Add the Spotify podcast rich link plus the shared remote-cache policy coverage needed for public-direct metadata fetches and cached content images.
- [x] Update `AGENTS.md` and `docs/openclaw-update-crud.md` so upstream checkouts automatically implement shared rich-enrichment fixes while preserving no-bypass, no-secret, and explicit auth-action safeguards.
- [x] Verify the change with rich-enrichment/build commands plus the repo-required lint, typecheck, and test suite, then record downstream impact and residual risk.

### Completion Review

- Result: Added the upstream `spotify-podcast` rich link, extended the shared remote-cache policy to allow `open.spotify.com` rich metadata fetches plus `i.scdn.co` content-image caching, and updated both `AGENTS.md` and `docs/openclaw-update-crud.md` so upstream checkouts automatically implement shared rich-enrichment fixes while preserving the existing no-bypass, no-secret, and explicit auth-action safeguards. `bun run enrich:rich:strict` now reports `spotify-podcast: fetched (metadata_complete) [HTTP 200]`, and `images:sync` localized the Spotify art to `public/cache/content-images/58ae79ba201183642ed2dc74ea3ad3e55791ed94420ebad894f0ac78c0cf2d32.jpg`. As part of the required committed cache refresh, the content-image manifest also picked up the current Substack image asset and removed an unreferenced stale Lemonade image cache entry.
- Verification: `bun run enrich:rich:strict` passed. `bun run images:sync` passed. `bun run validate:data` passed. `bun run build` passed. `bun run quality:check` passed. `bun run biome:check` passed. `bun run studio:lint` passed. `bun run typecheck` passed. `bun run studio:typecheck` passed. `bun run --filter @openlinks/studio-api test` passed. `bun run studio:test:integration` passed. `git diff --check` passed.
- Residual risk: `validate:data` and `build` still warn about the pre-existing stale LinkedIn authenticated cache plus partial metadata on the existing Rumble and Lemonade links. The shared policy/docs changes benefit downstream forks and `open-links-sites`, but the new `spotify-podcast` row itself remains an upstream content choice and will not automatically appear in forks that keep their own `data/links.json`.

## task-public-audience-delayed-retries | 2026-05-24 09:00 CDT | Add delayed retries for public audience capture

- [x] Add default two-retry delayed retries for nonfatal `public:rich:sync` audience failures, with CLI overrides and no retries for fatal profile-unavailable failures.
- [x] Cover retry success, retry exhaustion, fatal no-retry, retry disable, and injected-delay behavior with focused unit tests.
- [x] Verify with focused public-sync/workflow tests, deploy tests, typecheck, rich-enrichment diagnostics, and data validation.
...
## task-bright-builds-length-remediation | 2026-08-01 11:23 CDT | Eliminate Bright Builds length findings

- [x] Normalize the active lesson ledger so the managed lesson check passes without changing lesson meaning or order.
- [x] Split all 35 oversized implementation, test, and CSS files into cohesive modules without exceptions or public-contract changes.
- [x] Run the full repository verification matrix, review the complete diff for compatibility and fork-owned paths, and create one final commit on `main` without pushing.

### Completion Review

- Result: Normalized all 19 active lesson blocks and split every oversized implementation, test, and CSS file into focused modules behind compatibility facades. Bright Builds now scans 602 files with zero file-length or lesson findings and zero exceptions. Ownership-aware quality audits now follow the extracted route and style modules. Existing schemas, module paths, exports, CLI entrypoints, CSS selectors/cascade, and downstream-sensitive contracts remain intact; no personalized data, generated cache, or screenshot baseline is included.
- Verification: `bun scripts/bright-builds-check.ts all`, `bun run audit:fetch-cache`, `bun run validate:data`, `bun run enrich:rich:strict`, `bun run quality:check`, `bun run biome:check`, `bun run studio:lint`, `bun run typecheck`, `bun run studio:typecheck`, `bun run studio:web:test`, `bun run --filter @openlinks/studio-api test`, `bun run studio:test:integration`, `bun run test:deploy`, `bun run build`, `bun run test:playwright:navigation-menu`, `bun run test:playwright:payment-qr`, changed-function length audit, runtime import-cycle audit, export-facade review, fork-owned path review, and `git diff --cached --check` passed.
- Residual risk: Validation and build retain pre-existing warnings for stale Facebook and LinkedIn authenticated caches, partial Rumble and Lemonade metadata, an upstream Facebook image returning HTTP 403 while the committed cache is reused, a large Vite chart chunk, and stale Browserslist data. The managed Bright Builds checker remains excluded from local Biome traversal because its canonical upstream copy does not match this repo's current formatter; a targeted upstream Bright Builds formatting fix is recommended so the ignore can later be removed.

## task-non-mutating-build-content-refresh | 2026-08-01 12:51 CDT | Separate committed content refresh from build

- [x] Add an explicit, ownership-guarded content refresh orchestrator while preserving every existing leaf command and output contract.
- [x] Make local development, builds, hooks, CI, screenshots, and deploy artifact assembly consume committed content without tracked-file mutations.
- [x] Extend nightly follower-history automation to commit all allow-listed refreshed content before building and deploying the exact commit.
- [x] Update operator guidance and verify non-mutation, refresh ordering, upstream/fork deployment behavior, and the complete repository check matrix.

### Completion Review

- Result: Added the explicit `content:refresh` orchestrator and ownership manifest, removed implicit `predev`/`prebuild` refresh, made standard/strict/hook/screenshot/deploy builds consume committed content, and extended the existing nightly follower workflow to refresh, allow-list stage, commit, build, push, and deploy one snapshot. Existing leaf commands, schemas, deploy summary fields, CLI paths, and downstream renderer conventions remain intact; `--skip-content-sync` is a deprecated no-op.
- Verification: Focused refresh/build/workflow tests passed; `bun run content:refresh` passed all seven phases and rejected no unexpected paths. The tracked-state audit proved standard build, hook build, screenshot preparation, and deploy artifact assembly made no tracked changes; strict build also made no tracked changes but exited during strict validation. Bright Builds, fetch-cache audit, standard validation, strict enrichment, Biome, root/Studio lint and typechecks, Studio web/API/integration tests, 177 deployment tests, required CI, all three Studio Docker builds, standard build, and both Playwright suites passed.
- Residual risk: `build:strict` remains blocked by the pre-existing content state: stale credentialed Facebook and LinkedIn authenticated caches plus partial Rumble and Lemonade metadata are six strict-blocking warnings. Authenticated-cache refresh is intentionally outside general content refresh and requires the separate credentialed workflow; no fork-owned content or generated refresh drift was included in this implementation commit.
